...
const dirIndex = (from: Point, to: Point) => {
  const dx = to.x - from.x, dy = to.y - from.y;
  for (let i = 0; i < 8; i++) if (NEIGHBOURS[i].x === dx && NEIGHBOURS[i].y === dy) return i;
  return 0;
};
...
export function traceContour(mask: Uint8Array, width: number, height: number): Point[] {
  ...
  for (let y = 0; y < height && !start; y++) {
    for (let x = 0; x < width; x++) if (mask[y * width + x]) { start = { x, y }; break; }
  }
  if (!start) return [];
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    for (let k = 1; k <= 8; k++) {
      ...
      if (at(nx, ny)) { found = { x: nx, y: ny }; break; }
    }
    if (!found) break;
    backtrack = current;
    current = found;
...
export function simplify(points: Point[], epsilon = 1.4): Point[] {
 ...
 */
export function contourToPoints(
  contour: Point[],
